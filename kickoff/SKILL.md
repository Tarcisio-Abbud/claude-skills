...
paleta da skill `despachar` (passo 5).
...
O check no menu É a autorização — execute na sequência, sem reconfirmar. O casamento
tarefa→mecanismo (paleta), a receita do `/goal`, as fronteiras dos mecanismos e o contrato
de `loop.md` vivem na skill `despachar`: leia
`/root/.claude/skills/despachar/SKILL.md` antes do primeiro despacho. Despachos que são
comandos nativos do usuário (`/goal`, workflow) não travam o fluxo — entram no relato final
como linha pronta para colar.
