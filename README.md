...
| `/tk:review` | Lens campaign over a delivered code or data slice — the second pair of eyes on top of the repo's mandatory review: five lenses picked per slice (mandatory ones by slice type), the nit/defect ruler, the re-lens loop on a 3-round budget, the design signal that sends two defects or a repeated mechanism to the user, and the attack inventory a clean round ships — model-invoked. Prose gets the mandatory review only. The site names the trigger items and the provenance of every threshold in `~/.claude/tk/review.md` |
