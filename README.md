...
| `/tk:review` | Lens campaign over a delivered slice — the second pair of eyes on top of the repo's mandatory review: trigger receipt, five lenses with distinct prompts (mandatory ones by slice type), the severity ruler (nit/defect), the re-lens loop with a 3-round budget, the design signal that sends two defects or a repeated mechanism to the user instead of another round, and the attack inventory a clean round ships — model-invoked. The site names the trigger items and the provenance of every threshold in `~/.claude/tk/review.md` |
