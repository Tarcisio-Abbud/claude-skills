...
| L152–153 | "**Done when:** the inventory is one comment on the slice's ticket, failing that the PR body, rejections first, and every finding is in it." | KEEP, rewritten | Every run, and its unattended branch. Post-review commit on PR #93 (D6, homeserver-ambiente#207): the ticket comment is now the primary landing spot, the PR body its fallback, and rejections lead the inventory so a reader hits them first. |
