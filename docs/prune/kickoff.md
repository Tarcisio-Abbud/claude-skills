...
# Second pass — what the `WINDOW.md` pass removed (2026-09-22, T449)

The reversal path for the second pass, whose verdicts are at the end of `kickoff-report.md`.
Line numbers are those of `tk/skills/kickoff/WINDOW.md` at commit `3de11df`
(`git show 3de11df:tk/skills/kickoff/WINDOW.md`). Every quote below is verbatim, joined across
the original's line wraps. A DROP goes back as its own sentence. A CLAUSE goes back into the
sentence named beside it.

## DROP — inline evidence

- 23–24: "What collects on it: on 2026-08-18 the wall landed mid-dispatch and killed the five
  runs then in flight. What survived was exactly what had been committed and pushed."
- 175–177: "Measured on one orchestrator: 59k at the open — a kickoff is born carrying the
  CLAUDE.md and the memory index — 113k at the cut, 154k at the first lens, 210-232k while the
  handoff was written."
- 286–290: "Measured 2026-09-03: a generation stopped a lane's tail citing this threshold two
  lines after writing "context at the cut: unread". Read afterwards from that session's own
  transcript, it had been at ~210-232k against a ceiling of ~150-200k: the call was RIGHT. A
  right answer wearing a rule's clothes is still the defect, and that is why this bullet
  exists rather than a ban on estimating."
- 300–302: "One real slice — one `tk-queue` subcommand, 8 commits, five files — closed its
  session at **372k**, roughly twice that ceiling, with no overflow and no compaction, on six
  correction cycles."
- 335–336: "Measured 2026-08-28: a `pack` reached **~130k at the cut with zero items
  dispatched**, leaving 20–70k — about one item — for the whole package."
- 370–372: "It is what made that package run three hours with nobody watching — it queued
  while the window was spent and dispatched at the reset."
- 429–430: "The second half of one lane in that package ran with no claim at all, and only the
  crossing found it."
- 496–498: "Run from a worktree before this was so, the tick missed the project pair holding
  300,000 and reported the 1,000,000 default (17/09, T421)."
- 505–507: "Before that guard, one package collected four lines saying its context had been
  emptied while its transcript held no compaction at all (19/09, T432)."
- 516: "That is the piece that was missing on 06/09."
- 565–567: "On the night of 06-07/09 the hook rejected the end of every turn — "the goal says
  kill them all; 6 pull requests are still open" — while the user's own rule was to leave them
  unmerged."

## DROP — no-op, duplicate, exposition, unexercised run

- 9–10: "Every rule here fires at a moment nobody gets to choose."
- 15–16: "It is the cheapest of the three, and it is what makes the other two worth writing."
- 181–182: "A package planned as though review rode along inside implementation is a package
  whose last third is unfunded."
- 218–219: "Above all four sit the server-side flags the whole coverage rides on, which can be
  withdrawn with no change of version."
- 308–313, the whole paragraph: "**Estimating the implementation before it runs**, when there
  is no subagent to measure: inside a package the question does not arise, because the
  orchestrator implements nothing inline and every implementation therefore has a run of its
  own to be measured against. Outside one — a session implementing in the parent — the measure
  is that parent's own context delta across the slice, read at the same close-of-item point as
  above. Carry the delta into the handoff; it is the only number a successor can plan the next
  slice from."
- 343–344: "Reading ~150–200k at a planning seam would authorise exactly the session that
  failed."
- 471–472: "A cold reader has both halves of the condition here and asks nobody."

## CLAUSE — cut from sentences that stay

- 31–32, after "written to no transcript": "(measured 2026-09-03 across 302 files: zero
  occurrences)".
- 100–101, after "owes five contents in the same field**": "— four of them new, and the fifth
  one of the eight above doing a second job there. The section below names them."
- 128–131: "because the successor needs it before there is a map to read: in the state where
  nothing has merged yet, a map pinning the branch would be empty, and `git -C
  "<path>/spec-<m>" fetch --prune origin` — the successor's first command — could not be
  composed."
- 169–170, after "one agent's window": "; measured 2026-08-28, that was 144k subagent tokens
  against a 1,534-line slice".
- 201–202: "Measured 2026-09-19, it fired" and "in a `tmux` session over SSH,".
- 221–223, after "is unmeasured": ". The instrument that read every fact above cannot see such
  a warning, so its silence is evidence of nothing, and a package waiting for one waits on a
  promise nobody made."
- 265–266: "and the orchestrator measured on 2026-08-18 closed at 410k: there is no
  orchestrating a package from inside the smart zone".
- 279–280, after "an orchestrator's context": ", and three generations in a row reported the
  number as estimated, unread or `n/m` while this rule asked for it".
- 317–318, after "none of the graph is its own turn": ", and one package spent its whole window
  in that stretch with the quota unread".
- 343, after "a kickoff is born carrying": " (measured 2026-09-21: a cut read 111k with nothing
  wasteful in it)".
- 402–403, after "what moves between packages": " — nine live sessions and an empty container
  read the same 2 otherwise, and one of the two readings OOMs".
- 458–459, after "exactly the mechanism that failed": ", and Remote Control shows no
  statusline, so it could not be confirmed from outside either".
- 470–471, after "rather than ending the package": " — measured, the orchestrator of 05-07/09
  crossed two days at ~10% of context on three compacts".
- 476–477, before "written by hand the key": "Measured on the shipped binary, 2.1.263 on 08/09
  and 2.1.266 on 09/09:".
- 517–519: "for the same reason the bin above does: measured on 19/09, this hook fired inside
  an implementer that had just compacted and told it to read the orchestrator's handoff before
  anything else — an instruction to drop its own item."
- 567–568: "The goal was unsatisfiable by construction, and the hook only burned."
