...
leave the guard silently inert. A copy is a snapshot: after pulling a change to the script,
run the two `install` lines again.

**Private names beyond the config values** come from a file kept OUTSIDE this repo, one name
per line, `#` comments and blank lines ignored. Point the clone at it once:

```bash
git config tk.privateValuesFile <absolute path to the names file>
```

The authoring machine generates that file from its own list of private names, so this repo
never holds the list. Unset, the key protects nothing more. Set, with no readable file behind
it, **every commit is refused** until the file is back or the key is unset: the key promises
protection, and a guard that turned itself off when its list vanished would break that promise
silently.

**What it catches, each proved by a test that fails when the check is removed.** Its needles
are the values of `tk.tracker` and `tk.ghConfigDir` and every name in the file above. It reads four
...
- **It knows a list, not a category.** It greps the two config values and the names in the
  file, following encoding two levels deep and no further. A name the file does not list, and
  a ticket number of the private tracker, is equally unwelcome here and invisible to it. A
  needle of six letters and digits or more is compared with its separators dropped, as the
  `gh` publish guard compares the same list; a shorter one only as listed, except in a branch
  name.
