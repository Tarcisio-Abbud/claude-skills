...
class TheLaneRestart(DocTest):
    ...
    def test_the_suite_is_detached_and_waited_on_as_the_contract_says(self):
        block = " ".join(" ".join(self.block()).split())
        self.assertIn("detached and waited on in the FOREGROUND", block)
        self.assertIn("LANE-CONTRACT.md per-slice step 4", block,
                      "the suite outlasts the 600 s cap; a restart that orders it in "
                      "the foreground under one timeout orders a call the cap kills")
