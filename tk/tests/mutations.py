...
Run: python3 tk/tests/mutations.py --shard K/4, for K = 1..4, each its own call

The whole list no longer fits one 600-second foreground call, so it is replayed
in four shards; with no `--shard` the script replays the whole list at once.
Each shard reruns the baseline (~130 s alone) and then ~1.3 s per named test.
...
MUTATIONS = [
    ...
    # --- T455: the shards that make the whole run fit a foreground call ------
    ("T455 the first entry is replayed by every shard",
     "whole run's.\"\"\"\n    return [entry for i, entry in enumerate(mutations) if i % n == k - 1]",
     "whole run's.\"\"\"\n    return [entry for i, entry in enumerate(mutations)"
     " if i % n == k - 1 or i == 0]",
     ["TestMutationHarness.test_every_entry_lands_in_exactly_one_shard"],
     os.path.join("tests", "mutations.py")),

    ("T455 a shard outside 1..N is accepted and replays nothing",
     "    if not sep or not 1 <= k <= n:\n        raise argparse",
     "    if not sep:\n        raise argparse",
     ["TestMutationHarness.test_a_shard_outside_one_to_n_is_refused"],
     os.path.join("tests", "mutations.py")),

    ("T455 every shard judges the list-wide checks, not only shard 1",
     "    list_wide = shard_k == 1\n    entries = shard_of(",
     "    list_wide = True\n    entries = shard_of(",
     ["TestMutationHarness."
      "test_a_shard_other_than_the_first_does_not_evaluate_the_list_wide_checks"],
     os.path.join("tests", "mutations.py")),

    ("T455 the misnamed ratchet counts only the shard's slice",
     "    for entry in MUTATIONS:\n        problem = entry_problem(",
     "    for entry in entries:\n        problem = entry_problem(",
     ["TestMutationHarness."
      "test_the_first_shard_counts_the_misnamed_ratchet_over_the_whole_list"],
     os.path.join("tests", "mutations.py")),

    ("T455 a run with no --shard replays only a slice",
     ".shard or (1, 1)\n    sharded = ", ".shard or (1, SHARDS)\n    sharded = ",
     ["TestMutationHarness.test_no_shard_replays_the_whole_list_as_before"],
     os.path.join("tests", "mutations.py")),
    ...
    ("T216 `migrate` moves a block the log already carries a second time",
     ...
     ["TestAnInterruptedCloseIsFinishedNotRepeated."
      ...
      "TestAnItemsParagraphsLeaveWithIt."
      "test_a_rerun_of_an_interrupted_migrate_does_not_log_the_paragraphs_twice"]),
    ...
    # --- T452: an item's blank-separated paragraphs leave with it -----------
    ("T452 a close removes the item's block alone and strands its paragraphs",
     "    prose, left = item_extent(content, start)\n    whole = block + prose\n",
     "    prose, left = item_extent(content, start)\n    whole = block\n",
     ["TestAnItemsParagraphsLeaveWithIt.test_cancel_takes_the_paragraphs_to_the_done_log",
      "TestAnItemsParagraphsLeaveWithIt.test_done_takes_them_too_ahead_of_the_note",
      "TestAnItemsParagraphsLeaveWithIt."
      "test_the_replay_of_an_interrupted_close_takes_them_out_of_the_queue"]),

    ("T452 the paragraphs leave the queue but never reach the done-log",
     "    line += logged_prose(prose)", '    line += ""',
     ["TestAnItemsParagraphsLeaveWithIt.test_cancel_takes_the_paragraphs_to_the_done_log",
      "TestAnItemsParagraphsLeaveWithIt.test_done_takes_them_too_ahead_of_the_note"]),

    ("T452 the replay removes paragraphs the entry already written does not hold",
     "        if prose and not entry_holds_prose(log, args.id, prose):",
     "        if False:",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_a_replay_whose_entry_lacks_the_paragraphs_leaves_them_in_the_queue"]),

    ("T452 any entry of the ID counts as holding the paragraphs",
     "        if m and int(m.group(2)) == iid and lines[k + 1:k + 1 + len(want)] == want:",
     "        if m and int(m.group(2)) == iid:",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_a_replay_whose_entry_lacks_the_paragraphs_leaves_them_in_the_queue"]),

    ("T452 a paragraph in the item's prose no longer reaches its briefing",
     "                    handoff_candidates(iid, block))",
     '                    handoff_candidates(iid, block.split("\\n\\n", 1)[0]))',
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_a_briefing_the_items_paragraph_points_at_leaves_with_it"]),

    ("T452 bump moves the block and leaves its paragraphs under the item above",
     "    prose, left = item_extent(content, start)\n    block += prose\n",
     "    prose, left = item_extent(content, start)\n    block += ''\n",
     ["TestAnItemsParagraphsLeaveWithIt.test_bump_moves_the_paragraphs_with_the_item"]),

    ("T452 bump leaves prose behind in silence",
     '    print(f"{label} → top of the queue")\n    warn_prose_left(label, left)\n',
     '    print(f"{label} → top of the queue")\n',
     ["TestAnItemsParagraphsLeaveWithIt.test_bump_names_the_prose_it_leaves_behind"]),

    ("T452 migrate drops a [x] item's paragraphs from the queue and never logs them",
     '            done.append(t.rstrip("\\n") + logged_prose(prose) + "\\n")',
     "            done.append(t)",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_migrate_moves_a_checked_items_paragraphs_with_it"]),

    ("T452 migrate logs the paragraphs verbatim, not in the shape a close writes",
     '            done.append(t.rstrip("\\n") + logged_prose(prose) + "\\n")',
     "            done.append(t + prose)",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_migrate_moves_a_checked_items_paragraphs_with_it"]),

    ("T452 migrate leaves prose behind in silence",
     "    for name, left in left_behind:\n        warn_prose_left(name, left)\n",
     "    for name, left in left_behind:\n        pass\n",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_migrate_names_the_prose_a_checked_item_leaves_behind"]),

    ("T452 a heading no longer ends the item's prose run",
     '    if line.startswith("#"):\n        return True\n    if THEMATIC_BREAK_RE',
     "    if False:\n        return True\n    if THEMATIC_BREAK_RE",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_prose_that_runs_into_a_heading_stays_and_is_named"]),

    ("T452 a thematic break of `*` or `_` no longer ends the item's prose run",
     '    if THEMATIC_BREAK_RE.match(line.rstrip("\\r\\n")):',
     "    if False:",
     ["TestAnItemsParagraphsLeaveWithIt.test_every_thematic_break_is_a_rule"]),

    ("T452 a setext `===` underline no longer ends the item's prose run",
     '    return stripped != "-" and bool(SETEXT_UNDERLINE_RE.match(stripped))',
     "    return False",
     ["TestAnItemsParagraphsLeaveWithIt.test_every_thematic_break_is_a_rule"]),

    ("T452 no rule ends the item's prose run",
     ['    if THEMATIC_BREAK_RE.match(line.rstrip("\\r\\n")):',
      '    return stripped != "-" and bool(SETEXT_UNDERLINE_RE.match(stripped))'],
     ["    if False:", "    return False"],
     ["TestAnItemsParagraphsLeaveWithIt.test_prose_that_runs_into_a_rule_stays"]),

    ("T452 a lone `-` ends the item's prose run and strands its paragraphs",
     '    return stripped != "-" and bool(SETEXT_UNDERLINE_RE.match(stripped))',
     "    return bool(SETEXT_UNDERLINE_RE.match(stripped))",
     ["TestAnItemsParagraphsLeaveWithIt.test_a_lone_dash_is_not_a_rule"]),

    ("T452 prose running to the end of the file is taken as the last item's",
     "    if cut is None and i + 2 < len(blocks):", "    if cut is None:",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_prose_that_runs_to_the_end_of_the_file_stays_and_is_named"]),

    ("T452 the prose a move leaves behind stays in silence",
     "    was the item's, and a guess made in silence is the defect it replaces.\"\"\"\n    if left:",
     "    was the item's, and a guess made in silence is the defect it replaces.\"\"\"\n    if False:",
     ["TestAnItemsParagraphsLeaveWithIt."
      "test_prose_that_runs_into_a_heading_stays_and_is_named",
      "TestAnItemsParagraphsLeaveWithIt."
    ...
    ("T452 add and edit accept a line separator other than `\\n`",
...
def parse_shard(raw):
    """(k, n) from `K/N`, with 1 <= K <= N — the argparse type of `--shard`."""
    k, sep, n = raw.partition("/")
    try:
        k, n = int(k), int(n)
    except ValueError:
        k = n = 0
    if not sep or not 1 <= k <= n:
        raise argparse.ArgumentTypeError(f"{raw!r} is not K/N with 1 <= K <= N")
    return k, n


def shard_of(mutations, k, n):
    """The entries shard `k` of `n` replays: every n-th, starting at the k-th.

    Strided rather than cut into blocks, because the list is in the order the
    entries were written and neighbours share a suite region, so blocks would
    carry unequal costs. Each entry lands in exactly one shard, and the n shards
    together replay the whole list — which is why their caught counts sum to the
    whole run's."""
    return [entry for i, entry in enumerate(mutations) if i % n == k - 1]
...
def parse_args(argv):
    ...
    p.add_argument("--shard", type=parse_shard, metavar="K/N",
                   help=f"replay only shard K of N (the documented split is N={SHARDS}); "
                        "shard 1 also carries the list-wide checks")
    return p.parse_args(argv)


def main(argv=None):
    started = time.monotonic()
    # not `k, n`: the loop below binds `n` to each mutation's new text
    shard_k, shard_n = parse_args(argv).shard or (1, 1)
    sharded = shard_n > 1
    # THE LIST-WIDE CHECKS read the whole list, never a shard's slice of it: the
    # UNPROVED tests and the three KNOWN_* ratchets. They are static — no mutant
    # runs for them — so ONE shard carries them, over the full list, and every
    # other shard says it did not look, rather than passing a ceiling on the
    # fraction of the list it happened to hold.
    list_wide = shard_k == 1
    entries = shard_of(MUTATIONS, shard_k, shard_n)
    if sharded:
        print(f"SHARD {shard_k}/{shard_n}: {len(entries)} of {len(MUTATIONS)} entries "
              f"(entry i where i mod {shard_n} = {shard_k - 1}, counting from 0)\n")
    ...
    # indistinguishable from a mutant this run reddened. EVERY shard runs it: a
    # shard's kills mean nothing unless the suite was green where it ran.
    ...
    orphans = {name: unproved(per_module(MUTATIONS, name), obj)
               for name, obj in modules.items()} if list_wide else {}
    ...
    for entry in MUTATIONS:
        ...
        if rel not in sources:
            with open(os.path.join(TK_DIR, rel), encoding="utf-8") as f:
                sources[rel] = f.read()
    # the misnamed ratchet counts the WHOLE list, whatever slice this run replays;
    # over the whole run it is the same number the loop below reports entry by entry
    misnamed_count = 0
    for entry in MUTATIONS:
        problem = entry_problem(entry, modules,
                                sources[entry[4] if len(entry) > 4 else DEFAULT_SRC])
        misnamed_count += problem is not None and problem[0] == "MISNAMED"
    ...
    ran = len(entries) - len(unrunnable) - len(fictitious)
    print(f"\n{ran - len(survived)}/{ran} mutations caught"
          + (f" in shard {shard_k}/{shard_n}" if sharded else "")
          + (f" ({len(unrunnable)} could not run)" if unrunnable else "")
          + (f" ({len(fictitious)} name no such test)" if fictitious else "")
          + ((", " + ("list-wide: " if sharded else "")
              + " + ".join(f"{len(names)} in {module}"
                           for module, names in orphans.items())
              + " test(s) no entry proves") if list_wide else ""))
    if not list_wide:
        print(f"LIST-WIDE CHECKS (UNPROVED tests, the KNOWN_* ratchets): not evaluated "
              f"in this shard — they read the whole list, and shard 1/{shard_n} carries them")
    if sharded:
        print(f"shard {shard_k}/{shard_n} took {time.monotonic() - started:.0f} s")
    for title, items in (("SURVIVORS (the suite does not actually protect these)", survived),
        ...
        if items:
            ...
            for i in items:
                print("  -", i)
    grown = []
    for name, ceiling, count, what in () if not list_wide else (
            ("KNOWN_MISNAMED", KNOWN_MISNAMED, misnamed_count, "misnamed entries"),
