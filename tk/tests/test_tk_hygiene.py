...
# `gh api graphql` (the finished parents) answers from `<slug>.issues` instead:
# no file is a repository with no open issue, and a file reading FAIL fails,
# writing to stderr whatever follows FAIL on that line (a 502 when nothing does)
FAKE_GH = r"""#!/usr/bin/env python3
import os, sys
table = os.environ["FAKE_GH_TABLE"]
with open(os.path.join(table, "calls"), "a", encoding="utf-8") as f:
    f.write(" ".join(sys.argv[1:]) + "\n")
if "graphql" in sys.argv[1:]:
    field = dict(a.split("=", 1) for a in sys.argv[1:]
                 if a.startswith(("owner=", "name=")))
    answer = os.path.join(table, field["owner"] + "__" + field["name"] + ".issues")
    if not os.path.isfile(answer):
        sys.exit(0)
    with open(answer, encoding="utf-8") as f:
        text = f.read()
    if text.startswith("FAIL"):
        sys.stderr.write((text[len("FAIL"):].strip() or "HTTP 502: Bad Gateway") + "\n")
        sys.exit(1)
...
class HygieneTest(unittest.TestCase):
    ...
    def issues(self, slug, text):
        """What the fake `gh api graphql` answers for `slug`: jq's own lines."""
        with open(os.path.join(self.table, slug.replace("/", "__") + ".issues"),
...
class TestForgeAudit(HygieneTest):
    ...
    def test_a_forge_that_answers_an_error_is_unknown_and_exits_3(self):
        ...
        # the fake's error echoes the slug, as gh's does: the detail must not
        self.assertNotIn("example-owner/absent-repo", r.stdout)
...
class TestNoNetwork(HygieneTest):
    ...
    def test_one_slug_is_asked_for_its_finished_parents_once(self):
        # the same two trees, and the GraphQL read of the finished parents: a
        # paginated walk of every open issue, so the second ask is the costly one
        self.repo("clone-a", origin="https://github.com/example-owner/green-repo.git")
        self.repo("clone-b", origin="https://github.com/example-owner/green-repo.git")
        self.forge("example-owner/green-repo", "true")
        r = self.run_hygiene()
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        asked = [c for c in self.calls()
                 if "graphql" in c.split() and "name=green-repo" in c.split()]
        self.assertEqual(len(asked), 1, self.calls())
...
# --- the finished parents: named, never closed ------------------------------

class TestFinishedParents(HygieneTest):
    SLUG = "example-owner/tracker-repo"

    def tracker(self):
        path = self.repo("tracker",
                         origin=f"https://github.com/{self.SLUG}.git")
        self.forge(self.SLUG, "true")
        return path

    def parent_lines(self, out):
        return self.branch_lines(out, os.path.join(self.tmp, "tracker"),
                                 section="## finished parents")

    def test_only_the_parent_whose_sub_issues_all_closed_is_named(self):
        # #290 is the finished parent; #12 still has a child open; #40 has no
        # sub-issue at all, which is every ordinary issue and never a parent
        self.tracker()
        self.issues(self.SLUG, "290\t3\t3\n12\t1\t2\n40\t0\t0\n")
        r = self.run_hygiene()
        lines = self.parent_lines(r.stdout)
        self.assertEqual(lines, ["#290  all 3 sub-issue(s) closed"], r.stdout)
        self.assertNotIn(self.SLUG, r.stdout)
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)

    def test_the_block_is_silent_when_no_parent_is_finished(self):
        self.tracker()
        self.issues(self.SLUG, "12\t1\t2\n")
        r = self.run_hygiene()
        self.assertNotIn("## finished parents", r.stdout)
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)

    def test_a_forge_that_could_not_answer_is_named_not_silent(self):
        self.tracker()
        self.issues(self.SLUG, "FAIL\n")
        r = self.run_hygiene()
        lines = self.parent_lines(r.stdout)
        self.assertEqual(len(lines), 1, r.stdout)
        self.assertTrue(lines[0].startswith("not read"), lines)
        self.assertIn("502", lines[0])
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)

    def test_a_forge_error_naming_the_repository_does_not_leak_the_slug(self):
        # GitHub's own GraphQL error for a repository it cannot see, echoed in
        # the forge's case rather than the remote's: the detail keeps the error
        # and drops the slug, whatever its case
        self.tracker()
        self.issues(self.SLUG, "FAIL Could not resolve to a Repository with the name "
                               f"'{self.SLUG.upper()}'.\n")
        r = self.run_hygiene()
        lines = self.parent_lines(r.stdout)
        self.assertEqual(len(lines), 1, r.stdout)
        self.assertIn("Could not resolve to a Repository", lines[0])
        self.assertNotIn(self.SLUG, r.stdout.lower())
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)

    def test_an_answer_of_the_wrong_shape_is_not_read_as_no_parent(self):
        self.tracker()
        self.issues(self.SLUG, "290\tnull\t3\n")
        r = self.run_hygiene()
        lines = self.parent_lines(r.stdout)
