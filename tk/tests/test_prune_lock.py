...
THREE ROLES. A `pruned` row is a file a pruning pass rewrote, and a `moved` row a
file such a pass created to hold what it took out; both name the report that pass
wrote. A `guarded` row is a file no pass has pruned yet, named by the verdict that
wrote the lock and locked as found so it cannot drift while it waits for one — `kickoff/WINDOW.md`, unlocked, went 2753 -> 6794 body
words in three weeks. It names no report, because there is none. `docs/prune/lock.py`
carries the three tables and the provenance of every row.

A ROW LOCKED LATER carries its own `"locked"` date. `lock.py --add` writes such a
row, re-measuring one file after the baseline document and leaving every other row
as it was, so the numbers that document holds for it are no longer the lock's. The
full regeneration of 2026-09-23 (T443) folded every such row back into one baseline.
...
REGENERATE = ("when the growth is earned, re-lock that one file with "
              "`python3 docs/prune/lock.py --add <date> <file> > prune-lock.new && "
              "mv prune-lock.new tk/tests/prune-lock.json` — `--add` reads the lock, so "
              "a redirect straight into it empties it first (the whole lock: "
              "`python3 docs/prune/lock.py <date> > tk/tests/prune-lock.json`) — and "
...
class LockTest(unittest.TestCase):
    ...
    Three tests ask about every locked file; measuring per test would run the bin
    three times per file to learn the same numbers.
    """
...
class TestTheLockIsWellFormed(LockTest):
    ...
    def test_the_locked_numbers_are_the_committed_baseline_s(self):
        ...
        the `tk/reference` rows are absent from its table by construction. Every
        other row must be in the table — a count floor here instead would turn
        each `--add` of a baseline row into a red suite."""
        ...
        for path, locked in self.lock["files"].items():
            ...
            if "locked" in locked:
                continue        # numbers taken after this baseline, on the `locked` date
            if path.startswith("tk/reference/"):
                continue        # outside what `baseline.py` walks
            name = locked.get("baseline")
            key = f"tk/skills/{name}" if name else path
            compared += 1
            with self.subTest(file=path):
                self.assertTrue(key in table, f"{path} (baseline row `{key}`) is locked and "
                                          f"absent from {self.lock['baseline']} — {REGENERATE}")
                self.assertEqual(table.get(key), (locked["lines"], locked["body_words"]),
                                 f"{path} (baseline row `{key}`): the lock and "
                                 f"{self.lock['baseline']} disagree — {REGENERATE}")
        self.assertGreater(compared, 0, "no locked skill file was compared against "
                                        "the baseline table")

    def test_every_row_names_a_report_that_is_there(self):
        """The report is a pruned or moved row's provenance, and for such a row
        locked later it is the ONLY one: the baseline comparison above skips it. A
        row naming a report that is not on disk would hold numbers nobody can
        trace. A guarded row names none — its provenance is the baseline, or its
        `locked` date — and one that did would claim a pass that never ran."""
        for path, locked in self.lock["files"].items():
            with self.subTest(file=path):
                if "locked" in locked:
                    self.assertRegex(locked["locked"], r"^\d{4}-\d{2}-\d{2}$",
                                     f"{path}: `locked` is not a date")
                if locked["role"] == "guarded":
                    self.assertNotIn("report", locked,
                                     f"{path}: a guarded row names no report — a "
                                     f"pruned file moves to PRUNED in docs/prune/lock.py")
                    continue
                self.assertTrue(os.path.isfile(os.path.join(REPORTS, locked["report"])),
                                f"{path}: its report {locked['report']} is not in "
                                f"docs/prune")


class TestTheLockIsWhatLockPyPlans(LockTest):
    def test_the_lock_holds_exactly_the_rows_lock_py_plans(self):
        """The tables in `docs/prune/lock.py` are the list of what the lock holds, and
        the JSON is only their output. A row missing from the JSON is a file nothing
        holds any more — a hand edit, or an `--add` run after a table changed, drops
        one without a sound. A path in two tables comes out under the last one, and a
        row whose role or report drifted from its table claims a provenance it lacks.
        Compared as the map `path -> (role, report)`, both sides at once."""
        spec = importlib.util.spec_from_file_location(
            "prune_lock_generator", os.path.join(ROOT, "docs", "prune", "lock.py"))
        generator = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(generator)
        planned = generator.plan()
        paths = [path for path, _, _ in planned]
        twice = sorted({path for path in paths if paths.count(path) > 1})
        self.assertEqual(twice, [], f"in two tables of docs/prune/lock.py: {twice}")
        want = {path: (role, report) for path, role, report in planned}
        have = {path: (row["role"], row.get("report"))
                for path, row in self.lock["files"].items()}
        self.assertEqual(have, want, f"tk/tests/prune-lock.json and the tables of "
                                     f"docs/prune/lock.py disagree — {REGENERATE}")
