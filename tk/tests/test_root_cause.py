...
ROSTER = os.path.join(HERE, os.pardir, "bin", "tk-roster")
...
SECTIONS = ("The tracker import comes first",
            "The import and the other queues",
...
class TheTrackerImport(DocTest):
    def test_the_rule_names_which_ticket_becomes_an_item(self):
        ...
        for token in ("`ready-for-agent`", "**Parent**",
                      "has no item in any queue of this machine (next section)"):
            with self.subTest(token=token):
                self.assertIn(token, body)
...
def load_roster():
    """tk-roster as a module — it has no `.py` suffix, so the loader is named."""
    loader = importlib.machinery.SourceFileLoader("tk_roster_under_test", ROSTER)
...
class TheImportAndTheOtherQueues(DocTest):
    """T474. The import's "has no item in the queue" read only the orchestrator's own
    queue, so a ticket already an item in a SIBLING queue of the machine was imported
    a second time. The section prescribes one sweep over every queue; these checks pin
    its heading, run its command on a fixture HOME, and hold its glob to the roster's
    own definition of a queue."""

    TITLE = "The import and the other queues"

    def command(self):
        fence = re.search(r"```sh\n(.*?)```", self.section(self.TITLE), re.S)
        self.assertIsNotNone(fence, "the section prescribes no ```sh command")
        line = " ".join(fence.group(1).split())
        self.assertIn('"\\*\\*Ticket:\\*\\* <repo>#<n>\\b"', line,
                      "the sweep no longer matches the item's **Ticket:** field")
        return line

    def run_sweep(self, home, ticket):
        env = dict(os.environ, HOME=home)
        r = subprocess.run(["bash", "-c", self.command().replace("<repo>#<n>", ticket)],
                           env=env, capture_output=True, text=True, timeout=30)
        return sorted(r.stdout.split())

    def fixture(self):
        """Three projects: two queues whose tickets share a numeric prefix, and one
        finished project with a done-log and no queue."""
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        home = tmp.name
        projects = os.path.join(home, ".claude", "projects")
        files = {"-srv-a": ("next-steps.md", "- [ ] **T001** — a. **Ticket:** tracker#373.\n"),
                 "-srv-b": ("next-steps.md", "- [ ] **T002** — b. **Ticket:** tracker#37.\n"),
                 "-srv-c": ("done-log.md", "- **T003** — c. **Ticket:** tracker#37.\n")}
        for name, (fname, body) in files.items():
            os.makedirs(os.path.join(projects, name, "memory"))
            with open(os.path.join(projects, name, "memory", fname), "w",
                      encoding="utf-8") as f:
                f.write(body)
        return home, projects

    def test_the_section_follows_the_import_it_filters(self):
        order = [m.group(1) for m in re.finditer(r"^## (.+?)\s*$", self.text, re.M)]
        self.assertEqual(order[order.index("The tracker import comes first") + 1],
                         self.TITLE)

    def test_the_sweep_is_the_roster_s_and_runs_before_each_add(self):
        body = " ".join(self.section(self.TITLE).split())
        for token in ("before each `add`", "`tk-roster`",
                      "excluded and not dispatchable ones too"):
            with self.subTest(token=token):
                self.assertIn(token, body)

    def test_a_hit_is_not_imported_and_is_reported_with_its_queue(self):
        body = " ".join(self.section(self.TITLE).split())
        self.assertIn("A printed path means no `add`", body)
        self.assertIn("the path's queue on `AFK.md` step 6's skip line", body,
                      "a skipped ticket reported nowhere is a ticket that silently "
                      "vanishes from the package's account")

    def test_step_6_owns_the_skip_line_the_section_points_at(self):
        """The pointer once named step 6's audit block, which is AUDIT.md's wave block
        with four states and no slot for a skipped import: a pointer into silence."""
        six = re.search(r"^## 6\. .*?(?=^## 7\.)", self.afk, re.M | re.S)
        self.assertIsNotNone(six, "AFK.md lost step 6 or step 7")
        six = " ".join(six.group(0).split())
        self.assertIn("One **skip line** per ticket `ROOT-CAUSE.md`'s import left out", six)
        self.assertIn("naming the queue that holds it", six)
        self.assertIn("skip and deviation lines are written", six,
                      "step 6's Done when no longer counts the skip line it owes")

    def test_the_command_finds_the_ticket_in_whichever_queue_holds_it(self):
        home, projects = self.fixture()
        self.assertEqual(self.run_sweep(home, "tracker#373"),
                         [os.path.join(projects, "-srv-a", "memory", "next-steps.md")])
        self.assertEqual(self.run_sweep(home, "tracker#37"),
                         [os.path.join(projects, "-srv-b", "memory", "next-steps.md")],
                         "`#37` matched `#373`, or the done-log was swept as a queue")
        self.assertEqual(self.run_sweep(home, "tracker#3"), [])
        # the queue holds the repo lower-cased (tk-queue's canonical_ref); the tracker
        # may display it capitalised, and a missed match is a double import
        self.assertEqual(self.run_sweep(home, "Tracker#373"),
                         [os.path.join(projects, "-srv-a", "memory", "next-steps.md")],
                         "the sweep is case-sensitive, so a capitalised repo misses")

    def test_the_glob_is_the_roster_s_own_definition_of_a_queue(self):
        home, projects = self.fixture()
        glob = self.command().split()[-1]
        r = subprocess.run(["bash", "-c", f"ls -1 {glob}"], env=dict(os.environ, HOME=home),
                           capture_output=True, text=True, timeout=30)
        roster = load_roster()
        expected = sorted(os.path.join(memdir, roster.QUEUE_FILE)
                          for _, memdir in roster.queues(projects))
        self.assertEqual(len(expected), 2, "the fixture lost a queue")
        self.assertEqual(sorted(r.stdout.split()), expected)
