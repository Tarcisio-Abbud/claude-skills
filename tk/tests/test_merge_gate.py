#!/usr/bin/env python3
"""Doc-conformance proof for three rules of `../skills/merge-gate/SKILL.md`.

Run: python3 -m unittest discover -s tk/tests   (stdlib only, no deps)

Verdict 5 has a suite of its own (`test_tk_closure.py`, beside the checker that answers it).
What is asked here is what no bin can answer, each rule born of a measured merge:
...
- **the post-merge step of a live-consumed repository** (T435). The gate says to run it, and
  WHERE it says so is the rule: the attended menu is the only place in this file where such a
  merge executes, the strict form deferring every one of them to the user. So that check cuts
  the menu, and a sentence that drifted out of it reads as missing.
...
MENU_HEADING = re.compile(r"^## The menu, and what stays behind\s*$", re.M)
...
def section(text, heading):
    """The section `heading` opens, up to the next `## ` heading."""
...
def strict_form(text):
    """*The strict form*, the unattended gate."""
    return section(text, STRICT_HEADING)


def menu(text):
    """*The menu, and what stays behind* — the attended gate, where a merge runs."""
    return section(text, MENU_HEADING)
...
class MergeGateTest(unittest.TestCase):
    ...
    def test_the_slices_every_check_below_cuts_are_still_there(self):
        ...
        self.assertIn("AskUserQuestion", menu(self.text),
                      "the menu section no longer carries the attended gate's execution "
                      "point, and the placement check below would be reading nothing")
    ...
    # --- T435: a post-merge step for a repo consumed live --------------------

    def test_a_merge_of_a_live_consumed_repo_runs_the_sites_post_merge_step(self):
        """Verdict on ambiente#294: no automatic pull; a pull step in the merge
        flow instead. The gate names no server path of its own — it points at
        the site extension that does, as the strict form's live-consumed-repo
        rule already does two sections down."""
        said = flat(menu(self.text))
        self.assertIn(
            "After merging a repository this machine consumes live, run the "
            "post-merge step the site names in `~/.claude/tk/wrap-up.md`.",
            said,
            "the attended gate no longer sends a merge of a live-consumed "
            "repository to the site's post-merge step — the pull step from the "
            "#294 verdict has nowhere to run from. The slice is the MENU: the "
            "strict form merges none of those repositories itself, so a "
            "sentence sitting there fires for nobody")
