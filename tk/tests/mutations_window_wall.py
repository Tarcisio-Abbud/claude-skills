...
SIX ENTRIES MUTATE PROSE and five mutate `bin/tk-queue`, which is the default source. Prose is
...
harmed, and only a rule about the prose can catch that.
"""
...
MUTATIONS = [
    ...
    ("T265 the wall's step 2 writes the briefing and never says to run the printed `edit`",
     ", **then run the `edit` it prints** (same file, *The item points at the\n   briefing*)",
     "",
     [READS, RUNS], WINDOW),
    ...
    ("T265 the pointer names a section that does not exist",
     "(same file, *The item points at the\n   briefing*). That warning",
     "(same file, *The briefing points at the\n   item*). That warning",
     [POINTS], WINDOW),
