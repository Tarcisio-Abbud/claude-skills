...
class TestAnItemsParagraphsLeaveWithIt(QueueTest):
    """T452. A hand-written item can carry paragraphs below it, separated by a blank
    line — the blank that ends the item's BLOCK for every reader of its fields. A
    close removed the block and left the paragraphs in the queue, where they read as
    the item above's: measured on a real queue, where `cancel T043` left two of its
    paragraphs under T032 and out of the done-log.

    The format cannot tell an item's paragraph from the section's own prose, so the
    rule is positional: prose between two items is the item above's, and moves with
    it; prose that runs into a heading, a rule or the end of the file stays, and the
    command that left it SAYS so."""

    P1 = ("**Evidência já levantada — não re-derivar no grilling:** (1) o cronograma "
          "está transcrito à mão.\n")
    P2 = ("**Decidido em 2026-08-13: o grilling roda em SESSÃO NOVA** (contexto "
          "limpo).\n")
    PARAS = "\n" + P1 + "\n" + P2 + "\n"

    def entry_lines(self, label):
        """The done-log entry of `label`: its head line and every line under it."""
        lines = self.body("done-log.md").splitlines()
        at = next(i for i, ln in enumerate(lines) if f"— {label} " in ln)
        tail = []
        for ln in lines[at + 1:]:
            if not ln.startswith("  "):
                break
            tail.append(ln)
        return lines[at], tail

    def test_cancel_takes_the_paragraphs_to_the_done_log(self):
        self.seed(item(1, "um") + self.PARAS, item(2, "dois"))
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois"))
        head, tail = self.entry_lines("T001")
        self.assertIn("DESCARTADO", head)
        self.assertEqual(tail, ["  " + self.P1.rstrip("\n"), "  " + self.P2.rstrip("\n")])

    def test_done_takes_them_too_ahead_of_the_note(self):
        self.seed(item(1, "um") + self.PARAS, item(2, "dois"))
        r = self.run_tk("done", "T001", "--how", "feito", "--note", "nota do fecho")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois"))
        _, tail = self.entry_lines("T001")
        self.assertEqual(tail, ["  " + self.P1.rstrip("\n"), "  " + self.P2.rstrip("\n"),
                                "  nota do fecho"])

    def test_the_replay_of_an_interrupted_close_takes_them_out_of_the_queue(self):
        """The log half was written, paragraphs included, before the crash; the
        replay finishes the queue half and must not strand them there."""
        log = "- 2026-08-01 — FEITO — T001 um — PR #1\n  " + self.P1 + "  " + self.P2
        self.seed(item(1, "um") + self.PARAS, item(2, "dois"), log=log)
        r = self.run_tk("done", "T001", "--how", "PR #2")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertIn("already has a done-log entry", r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois"))
        self.assertEqual(self.body("done-log.md"), log)
        self.assertNotIn("prose", r.stderr)

    def test_a_replay_whose_entry_lacks_the_paragraphs_leaves_them_in_the_queue(self):
        """An entry written without them — a crash under the code before T452, a
        hand-written line, or another item's entry colliding on the ID — does not
        hold them, and the queue is then their only copy. Removing them there loses
        them; they stay, and the replay says so."""
        head = "- 2026-08-01 — FEITO — T001 um — PR #1\n"
        for log in (head, head + "  " + self.P1):
            with self.subTest(log=log):
                self.seed(item(1, "um") + self.PARAS, item(2, "dois"), log=log)
                r = self.run_tk("done", "T001", "--how", "PR #2")
                self.assertEqual(r.returncode, 0, r.stderr)
                self.assertIn("already has a done-log entry", r.stderr)
                self.assertEqual(self.body(), HEADER + self.P1 + "\n" + self.P2 + "\n"
                                 + item(2, "dois"))
                self.assertEqual(self.body("done-log.md"), log)
                self.assertIn("2 lines of prose right after T001", r.stderr)
                self.assertIn("does not carry them", r.stderr)

    def test_bump_moves_the_paragraphs_with_the_item(self):
        self.seed(item(1, "um") + "\n", item(2, "dois") + self.PARAS, item(3, "tres"))
        r = self.run_tk("bump", "T002")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois") + self.PARAS
                         + item(1, "um") + "\n" + item(3, "tres"))

    def test_migrate_moves_a_checked_items_paragraphs_with_it(self):
        # born: an unstamped item would be backdated, and this is not that test
        two = item(2, "dois", born="2026-08-13")
        self.seed("- [x] **T001** — feito à mão\n" + self.PARAS, two)
        r = self.run_tk("migrate")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + two)
        log = self.body("done-log.md")
        # the shape a close writes: indented under the head, blank lines dropped
        self.assertIn("- [x] **T001** — feito à mão\n  " + self.P1 + "  " + self.P2, log)
        self.assertNotIn("prose", r.stderr)

    def test_a_rerun_of_an_interrupted_migrate_does_not_log_the_paragraphs_twice(self):
        two = item(2, "dois", born="2026-08-13")
        self.seed("- [x] **T001** — feito à mão\n" + self.PARAS, two)
        self.assertEqual(self.run_tk("migrate").returncode, 0)
        log = self.body("done-log.md")
        # the crash: the log half written, the queue half not
        self.seed("- [x] **T001** — feito à mão\n" + self.PARAS, two)
        r = self.run_tk("migrate")
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_migrate_names_the_prose_a_checked_item_leaves_behind(self):
        prose = "Contexto da seção, que não é de item nenhum.\n"
        two = item(2, "dois", born="2026-08-13")
        self.seed("- [x] **T001** — feito à mão\n\n" + prose + "\n## outra seção\n\n", two)
        r = self.run_tk("migrate")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertIn(prose + "\n## outra seção\n", self.body())
        self.assertNotIn("Contexto da seção", self.body("done-log.md"))
        self.assertIn("1 line of prose right after T001", r.stderr)

    def test_bump_names_the_prose_it_leaves_behind(self):
        prose = "Pendência leve, fora de qualquer item.\n"
        self.seed(item(1, "um") + "\n", item(2, "dois") + "\n" + prose)
        r = self.run_tk("bump", "T002")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois") + "\n" + item(1, "um")
                         + "\n" + prose)
        self.assertIn("1 line of prose right after T002", r.stderr)

    def test_prose_that_runs_into_a_heading_stays_and_is_named(self):
        prose = "Contexto da seção, que não é de item nenhum.\n"
        self.seed(item(1, "um") + "\n" + prose + "\n## outra seção\n\n", item(2, "dois"))
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + prose + "\n## outra seção\n\n" + item(2, "dois"))
        self.assertNotIn("Contexto da seção", self.body("done-log.md"))
        self.assertIn("T001", r.stderr)
        self.assertIn("1 line of prose", r.stderr)

    def test_prose_that_runs_into_a_rule_stays(self):
        prose = "Contexto da seção, que não é de item nenhum.\n"
        self.seed(item(1, "um") + "\n" + prose + "\n---\n\n", item(2, "dois"))
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_every_thematic_break_is_a_rule(self):
        """A rule is a CommonMark thematic break — three or more of `*`, `-` or `_`,
        spaces allowed between them — not only the setext underline's `---`. The
        rest were read as the item's prose: a cancel logged the rule and the section
        below it under the item, and deleted both from the queue."""
        prose = "Contexto da seção, que não é de item nenhum.\n"
        for rule in ("***", "___", "- - -", "* * *", "_ _ _ _", "  ***  ", "---", "==="):
    ...
    def test_a_lone_dash_is_not_a_rule(self):
        """One `-` is neither a thematic break nor what the queue writes as a rule;
        between two items it is the item's prose, and leaves with it."""
        self.seed(item(1, "um") + "\nlista:\n-\nfim\n\n", item(2, "dois"))
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + item(2, "dois"))
        _, tail = self.entry_lines("T001")
        self.assertEqual(tail, ["  lista:", "  -", "  fim"])

    def test_a_briefing_the_items_paragraph_points_at_leaves_with_it(self):
        """The paragraphs are the item's, so a `[[handoff-T<id>]]` in them is the
        item's pointer: closing the item collects that briefing, as a pointer on
        its own line would. A file deletion — so pinned, both ways: the prose the
        item leaves behind is not its own, and its pointer collects nothing."""
        brief = "# Handoff T009 — campanha\n\nobjetivo\n"
        self.seed(item(1, "um") + "\nver [[handoff-T009]]\n\n", item(2, "dois"))
        self.write("handoff-T009.md", brief)
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertIn("handoff-T009.md removed", r.stdout)
        self.assertFalse(os.path.exists(os.path.join(self.mem, "handoff-T009.md")))
        # the section's prose is not the item's: its pointer holds nothing to go
        self.seed(item(1, "um") + "\nver [[handoff-T009]]\n\n## seção\n\n", item(2, "dois"))
        self.write("handoff-T009.md", brief)
        r = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertTrue(os.path.exists(os.path.join(self.mem, "handoff-T009.md")))

    def test_prose_that_runs_to_the_end_of_the_file_stays_and_is_named(self):
        """The shape of the workspace queue today: free prose after its last item."""
        prose = "Pendência leve nascida da faxina, fora de qualquer item.\n"
        self.seed(item(1, "um"), item(2, "dois") + "\n" + prose)
        quiet = self.run_tk("cancel", "T001", "--why", "obsoleto")
        self.assertEqual(quiet.returncode, 0, quiet.stderr)
        self.assertNotIn("prose", quiet.stderr)     # nothing after T001 but T002
        r = self.run_tk("cancel", "T002", "--why", "obsoleto")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(self.body(), HEADER + prose)
        self.assertNotIn("Pendência leve", self.body("done-log.md"))
        self.assertIn("T002", r.stderr)

    # every separator str.splitlines honours: split_blocks reads each as a line
    # break, and a bare `\r` pair as a BLANK line — the one line add and edit must
    # never write inside an item, since the extent rule above rests on it
    SEPARATORS = ("\r", "\r\r", "\r\n", "\x0b", "\x0c", "\x1c", "\x1d", "\x1e",
                  "\x85", " ", " ")

    def test_add_refuses_every_line_separator(self):
        for sep in self.SEPARATORS:
            with self.subTest(sep=repr(sep)):
                self.seed(item(1, "um"))
                r = self.run_tk("add", f"foo{sep}bar", "--class", "AUTONOMOUS",
                                "--effort", "S", "--criterion", "A: c")
                self.assertEqual(r.returncode, 1, r.stdout)
                self.assertIn("single-line", r.stderr)
                self.assertEqual(self.body(), HEADER + item(1, "um"))

    def test_edit_refuses_every_line_separator(self):
        for sep in self.SEPARATORS:
            with self.subTest(sep=repr(sep)):
                self.seed(item(1, "um"))
                r = self.run_tk("edit", "T001", "--criterion", f"A: foo{sep}bar")
                self.assertEqual(r.returncode, 1, r.stdout)
                self.assertIn("single-line", r.stderr)
                self.assertEqual(self.body(), HEADER + item(1, "um"))
...
class TestMutationHarness(unittest.TestCase):
    ...
    def test_every_entry_lands_in_exactly_one_shard(self):
        """The shards' caught counts are summed and read as the whole run's, which
        is true only if the shards partition the list: an entry in two shards is
        counted twice, and an entry in none is never replayed at all."""
        full = self.h.MUTATIONS
        for n in range(1, 8):
            with self.subTest(n=n):
                seen = [id(e) for k in range(1, n + 1)
                        for e in self.h.shard_of(full, k, n)]
                self.assertEqual(len(seen), len(full))
                self.assertEqual(sorted(seen), sorted(id(e) for e in full))
        self.assertEqual(self.h.shard_of(full, 1, 1), full)

    def test_a_shard_outside_one_to_n_is_refused(self):
        """`0/4` or `5/4` would replay no entry and print a clean `0/0`."""
        self.assertEqual(self.h.parse_shard("2/4"), (2, 4))
        for raw in ("0/4", "5/4", "4", "a/4", "2/", "-1/4"):
            with self.subTest(raw=raw):
                with self.assertRaises(self.h.argparse.ArgumentTypeError):
                    self.h.parse_shard(raw)

    def run_harness(self, *argv):
        """The harness's `main` over four entries, two per shard of two: one names
        no such test, one is a real mutant. The baseline is answered green and
        every mutant's test red, without running either."""
        import contextlib
        import io
        from unittest import mock
        shebang = "#!/usr/bin/env python3\n"
        mutant = ("a mutant", shebang, shebang + "# mutated\n",
                  ["TestPrefixedId.test_garbage_is_still_rejected"])
        entries = [("first", "a", "b", ["TestNoSuchClass.test_x"]),
                   ("second", "a", "b", ["TestNoSuchClass.test_y"]), mutant, mutant]

        def run_suite(tk_dir, names):
            return subprocess.CompletedProcess([], 0 if tk_dir == self.h.TK_DIR else 1)
        out = io.StringIO()
        with mock.patch.object(self.h, "MUTATIONS", entries), \
                mock.patch.object(self.h, "KNOWN_MISNAMED", 5), \
                mock.patch.object(self.h, "run_suite", run_suite), \
                contextlib.redirect_stdout(out):
            code = self.h.main(list(argv))
        return code, out.getvalue()

    def test_a_shard_other_than_the_first_does_not_evaluate_the_list_wide_checks(self):
        """The ratchets and the UNPROVED list read the whole list. A shard judging
        them on its slice would fail a ceiling that the whole run passes, or pass
        one it fails — so every shard but the first says it did not look."""
        code, out = self.run_harness("--shard", "2/2")
        self.assertIn("1/1 mutations caught in shard 2/2 (1 name no such test)\n", out)
        self.assertIn("not evaluated in this shard", out)
        for line in ("no mutation entry names this test", "the debt grew",
                     "lower the constant", "no entry proves"):
            self.assertNotIn(line, out)
        self.assertEqual(code, 0, out)

    def test_the_first_shard_counts_the_misnamed_ratchet_over_the_whole_list(self):
        """Shard 1 of 2 holds one of the two misnamed entries and must still
        count both: the ceiling is on the list's debt, not on the slice's."""
        code, out = self.run_harness("--shard", "1/2")
        self.assertIn("1/1 mutations caught in shard 1/2 (1 name no such test), list-wide:",
                      out)
        self.assertIn("this run found only 2 misnamed entries", out)
        self.assertIn("no entry proves", out)
        self.assertEqual(code, 1, out)   # two entries leave nearly every test unproved

    def test_no_shard_replays_the_whole_list_as_before(self):
        """With no `--shard` the run is the whole list, and its tally line is the
        one it printed before shards existed: no shard label, no list-wide tag."""
        code, out = self.run_harness()
        self.assertIn("\n2/2 mutations caught (2 name no such test), ", out)
        self.assertIn("this run found only 2 misnamed entries", out)
        for line in ("SHARD", "in shard", "list-wide:", "not evaluated", "took"):
            self.assertNotIn(line, out)
