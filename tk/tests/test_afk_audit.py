...
`TheWaveIsSharedAndScoped` pins the two headings a wave split across queues needs — the
lock on the spec's issue and the scope of the lenses — and AFK.md's pointer to both. It
reads them, never runs them: the lock is a tracker comment no fixture here can post.
...
class TheWaveIsSharedAndScoped(unittest.TestCase):
    """The two headings that let one spec feed packages in two queues.

    A claim guards one queue only, so nothing stopped two orchestrators from auditing
    one spec at once; and a lens reading every ticket of the wave commented tickets
    another package would implement unaudited. `AUDIT.md` carries the lock and the
    scope under their own headings, and AFK.md's audit step names both — an unnamed
    heading is one the step's reader never reaches.
    """

    HEADINGS = ("Two orchestrators, one wave", "What the lenses read")
    ...
    def test_audit_md_carries_each_heading_once(self):
        text = audit_text()
        for heading in self.HEADINGS:
            with self.subTest(heading=heading):
                found = re.findall(r"^## %s\s*$" % re.escape(heading), text, re.M)
                self.assertEqual(len(found), 1,
                                 f"AUDIT.md carries `## {heading}` {len(found)} times")

    def test_the_lock_names_the_marker_a_sibling_reads(self):
        """Both orchestrators read the same marker; renaming it on one side of a
        version skew hides a live audit from the other."""
        lock = self.section(self.HEADINGS[0])
        self.assertIsNotNone(lock, "AUDIT.md lost its lock heading")
        self.assertIn("`Wave audit <date> em curso — <queue dir>`", lock)
        flat_lock = " ".join(lock.split())
        self.assertIn("under 5h old", flat_lock,
                      "the lock no longer says when a comment is dead")
        self.assertIn("--paginate --jq", flat_lock,
                      "bare --paginate prints one JSON array per page, unparseable as one")

    def test_afk_md_step_4_points_at_both_headings(self):
        text = afk_text()
        m = AUDIT_STEP_HEADING.search(text)
        self.assertTrue(m, "AFK.md has no numbered Audit step")
        rest = text[m.end():]
        nxt = re.search(r"^## ", rest, re.M)
        ...
        for heading in self.HEADINGS:
            with self.subTest(heading=heading):
                self.assertIn(f"*{heading}*", flat_step,
                              f"AFK.md's audit step never names `{heading}`")
