...
class TestTheDispatchProseNamesTheCommands(unittest.TestCase):
    ...
    def test_the_attended_dispatch_names_the_contract_block_and_the_reference(self):
        ...
        self.assertIn("## 5. Dispatch", text,
                      "kickoff/SKILL.md has no step 5 — the assertions below read a "
                      "section that is gone")
        # Scoped to step 5, which is the last numbered step: it ends at the next
        # heading of any kind, not at a "## 6." that does not exist.
        step = text.split("## 5. Dispatch")[1].split("\n## ")[0]
        self.assertIn("tk-contract", step)
        # Anchored on the invocation shape, not the surrounding prose: the
        # dispatch step must show tk-ticket-ref actually called with
        # --closing-line inside one code span, which a line wrap may split.
        self.assertRegex(
            step, r"tk-ticket-ref[^`]*--closing-line",
            "the dispatch step must show tk-ticket-ref invoked with --closing-line")
