...
# The foreground wait on a detached suite. The suite outlasts the Bash tool's 600 s
# cap, so no single foreground call can hold it; the wait can, one bounded call at a time.
THE_WAIT = "timeout 590 sh -c 'until [ -s <rc> ]; do sleep 10; done'"
...
LANE_RULES = {
    ...
    "the suite runs detached": "echo $? > <rc>) > <log> 2>&1 &",
    "the wait is foreground and bounded": THE_WAIT,
    "the wait is re-issued on 124": "exits 124, re-issue it",
    "mutation run within the tool's cap": "600000 ms",
    ...
    "one worktree per lane branch": "no `isolation: 'worktree'`",
    "the reviewer shares the lane's worktree": "reviewer works in the implementer's worktree",
}
...
class TheLaneRules(DocTest):
    ...
    def test_no_contract_declares_a_timeout_past_the_tools_cap(self):
        for path in (LANE, REVIEW, os.path.join(KICKOFF, "LEDGER.md")):
            for seconds in re.findall(r"\btimeout (\d+)", read(path)):
                with self.subTest(file=os.path.basename(path), seconds=seconds):
                    self.assertLessEqual(int(seconds), 590,
                                         "one foreground call past 600 s is a call the "
                                         "Bash tool's cap kills; the suite is detached "
                                         "and waited on")
...
class TheDispatchPrompt(DocTest):
    ...
    def test_the_prompt_detaches_the_suite_and_waits_in_the_foreground(self):
        block = " ".join(" ".join(self.block()).split())
        self.assertIn("detached and waited on in the FOREGROUND", block,
                      "a prompt that orders the suite in the foreground under one "
                      "timeout orders a call the 600 s cap kills")
        self.assertIn("as per-slice step 4 says", block,
                      "the contract numbers three lists; a bare step 4 can land on "
                      "the notes input or the report")
...
class TheReviewerFixes(DocTest):
    ...
    def test_the_reviewer_runs_the_suite_in_the_foreground(self):
        body = self.steps()
        for term in ("detached and is waited on in the FOREGROUND",
                     "`LANE-CONTRACT.md` per-slice step 4"):
            with self.subTest(term=term):
                self.assertIn(term, body,
                              "the reviewer's suite outlasts the 600 s cap as the "
                              "lane's does, so it is detached and waited on by the "
                              "same step, which carries the commands")
        self.assertIn("waiting on a notification", body,
...
class TheCollisionCheck(DocTest):
    ...
    def test_the_check_points_at_the_merge_gate_and_does_not_restate_it(self):
        body = self.body()
        self.assertIn("`../merge-gate/DIGEST.md` item 5", body,
