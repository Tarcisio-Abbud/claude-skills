...
either, invoked directly. Its "Done when" is the caller's.

## The digest

**Every PR in the gate gets a digest before the menu, shaped by `DIGEST.md`** — including a
PR the verdicts hold back, where the decision is whether the red is worth fixing now. Its
block is the one thing per PR the gate prints, and it opens the PR body. Its last line
carries the five verdicts of **safe-to-merge**:
...
(required trailer lines; on the default branch, branch first). After merging a repository
this machine consumes live, run the post-merge step the site names in
`~/.claude/tk/wrap-up.md`. Every unchecked action enters the queue as a DECISION
...
entering the branch by its own `T<id>` merge commit. One pull request, so its trail
sections are written once, over the package — and N item blocks, because what the user
...
- **The digest's block opens the PR body**, because an unattended session has nobody at the
...
menu, in `DIGEST.md`'s shape: its block printed at an attended gate and opening the PR body,
lines 1 and 6 recomputed there, its collision with the other open PRs run for real, not read off
the forge. A package's accumulated lane
carried one digest per item, its verdicts split package/item and its
`--merge --delete-branch` precondition stated. Every merged PR's body describes what it
merged. Every merge of a repository this machine consumes live was followed by the site's
post-merge step, or the gate named that step as skipped. The user has the summary — what
