...
implementation. It is heavier than `/tk:wrap-up`, which covers only the session. Each step
ends on an **exhaustive** criterion: "every claim verified", not "a list of changes".

## 1. Inventory the docs' claims
Enumerate all documents: `README`, the project's instruction file (`CLAUDE.md`/`AGENTS.md`/
`GEMINI.md`), the glossary `CONTEXT.md`, `docs/`, ADRs. From each, extract the **verifiable
claims** — commands, file paths, counts (number of tests), script and function names,
described flows, decisions.
...
**Done when:** EVERY claim in the inventory is marked exact/stale.
...
Preserve legitimate history. A proposal or decision section in a design doc or ADR is not
rewritten to pretend the original plan was the final result.
**Done when:** no claim marked stale remains.

## 4. Verify
Run the project's test suite, **detecting the runner** rather than assuming one. Re-check the
...
- **Obsolete → propose pruning.** The fact stopped holding: the file/flag/script it cites no
  ...
  that against reality, not from memory: `grep` the path, `gh pr view`, `git log`. List the
  memory with the evidence that killed it. The user deletes — this step proposes and leaves
  every file on disk.
...
## 6. Open the PR
Create a branch and commit the doc fixes, plus any step-5 promotion that landed in this
repo, following the project's commit conventions. Open the PR with the audit summary: what
was stale, what was fixed. Don't touch production code — this audit is documentation-only.
**Done when:** the PR is open and referenced in the reply to the user.

## 7. Recommend the next step
After the audit the state is externalized by definition — docs match code, tests green — so
