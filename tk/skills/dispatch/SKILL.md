---
name: dispatch
description: "Pick the mechanism that runs a task unattended — `/loop`, `/schedule`, a subagent — and write the ready-to-paste line. Use when the mechanism is in doubt, or another skill needs the palette."
---
...
| Waiting on external state (CI, third party) | Monitor — a background script streaming the state; `/loop` on an interval where no command observes it | the agent |
...
A `/schedule` routine runs with no human and no permission prompt. Write it self-contained,
with the done criterion embedded and a smaller model for mechanical work.
...
prompt or the subagent at `skills/wrap-up/SKILL.md` of the `tk` plugin. Work that outlives one
run is a **generation** (`../kickoff/WINDOW.md`), opened by a scheduled fire or by the user's
own line. There the dispatch delivers the line the user types.
...
- **`/loop` over `loop.md`** — same-session slices, context accumulating across iterations;
