...
4. **Tabulate.** Write one row per sentence that instructs, defines a criterion or is evidence.
...
Step 3 ends when every rule has a named run. Step 4 ends when every such sentence, in every
...
- **DROP** names its reason: duplicate, evidence, a copy of the environment, unexercised run.
...
- The `description` of every file read is the one field pruned in place: its row carries the
  wording that replaces it.
- Exposition groups under the rule it explains: one row, its `where` column naming the range.
...
sentence its unit names, and the proof is named.
