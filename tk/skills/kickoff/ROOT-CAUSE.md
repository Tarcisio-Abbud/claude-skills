...
stands. The number is DERIVED, not measured: the one run (05–07/09/2026) read 110 items in ~40
min and returned 12 disjoint lanes, where the package that skipped it would have opened ~78 pull
requests into the same files. A package fits 3–6 items, so 20 is about four packages deep —
where 40 minutes starts buying more than it costs. The next run that measures the audit replaces
this derivation with its number.
...
and has no item in any queue of this machine (next section) becomes an item:
...
A ticket already held as an item in a sibling queue, imported again, is dispatched twice. So
before each `add`, sweep every queue `tk-roster` counts, excluded and not dispatchable ones too:

```sh
grep -liE "\*\*Ticket:\*\* <repo>#<n>\b" ~/.claude/projects/*/memory/next-steps.md
```

`<repo>#<n>` is what `--ticket` would write; `-i` forgives the tracker's casing, `\b` keeps `#37`
from matching `#373`. The glob is the roster's own definition of a queue, this one included. A
printed path means no `add`: report the ticket and the path's queue on `AFK.md` step 6's skip line.
...
"The tick" says why the two are not one number. `FlexibleOrchestrator`, `llm_call` and
`extract_xml` do not cross either ceiling, and for the same reason.
