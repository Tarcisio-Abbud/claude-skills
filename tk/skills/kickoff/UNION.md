...
last item is verified and BEFORE step 6 measures. `REVIEW-CONTRACT.md` beside this file names
half 1 as the collision check that closes the package: that is THIS run, not a second one —
one run per package, the orchestrator's, and a reviewer who reaches it from there reads the
answer rather than re-running the passes, whose suite per union is the expensive half.
...
first pass merges every PAIR for real, and `../merge-gate/DIGEST.md` item 5 owns that form. The
