...
3. **Verify by artifact.** After the fixes, re-run the suite and `git diff origin/main...HEAD
   --stat` on the final tree. **The suite runs detached and is waited on in the FOREGROUND**,
   with a mutation run only where a fix touched an anchor, both as `LANE-CONTRACT.md`
   per-slice step 4 says. Never end the turn waiting on a notification — an agent that waited
   on a background `pytest` returned twice with no pull request and no comment.
...
`../merge-gate/DIGEST.md` item 5 owns the first pass's argument form; `UNION.md` beside this file
