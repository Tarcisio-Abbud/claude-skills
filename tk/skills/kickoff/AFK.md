...
`tk-queue claim "<id>" --as "<session-or-host-label>"` — letters, digits, `.`, `-` or `_`, no
spaces, at most 32 chars — taken under the exclusive lock the queue already holds. A second
claim is REFUSED, naming the owner and the moment — and that refusal IS the guard: the item
leaves the package untouched and enters the report as held elsewhere.
