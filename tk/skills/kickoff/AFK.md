...
package. Read it there, whole, and hand its block to step 6. It runs once per wave, not once
per package: over a spec an earlier package already audited, report that audit's date and fire
no lens. Every `--criterion` the audit reads is anchored by `../verify/SKILL.md`, *The anchor
outlives the tree* — one of its three rotten shapes — and this is where the wording is still
cheap to change.
