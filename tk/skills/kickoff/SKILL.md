...
items, verifies and triages it, and **dispatches** what the user checks. Execute the steps in
...
**Open on a clean tree.** Run `../../bin/tk-hygiene` first — it audits every repo's
`delete_branch_on_merge`, prunes merged local branches, unforced, and the lane's remote
`spec/<m>/T<id>`; safe twice, asks nothing.
Exit 1 (`delete_branch_on_merge=false`): queue an item, never a question. Exit 3: name the repo it
could not audit and carry on. Exit 2: the run did not happen — the audit is unread, not clean.
Its `## finished parents` block names open issues with every sub-issue closed; read each body.
Work still in a body is a candidate item, and its parent stays open. With none, closing the
parent (`gh issue close`) is the candidate item, run on the user's check. Under `afk` or
...
literal `YYYY-MM-DD`), a short block of context, not agenda: it catches an item about
...
**Done when:** the hygiene result is in the report, each finished parent was routed, and the
user saw the week's closed items with a single list of candidate items, each with its source —
or a package handoff was found here, and its contents are the agenda.
...
bound to another environment (**Env**, read off the item: `tk-queue list` omits it) are never
options — the last go to block (d), though their DECISIONs stay in the menu: deciding is machine-agnostic.
...
Before firing a Workflow script from here, read `AFK.md`'s vehicle: it owns which worktree each
run works in — one worktree per lane branch, implementer and reviewer in it.
...
and flags: `tk-queue --help` and each subcommand's `--help`.
