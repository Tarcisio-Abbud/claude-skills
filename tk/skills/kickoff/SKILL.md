...
`delete_branch_on_merge` and prunes what the default has: a local branch whose remote is gone with
the worktree on it, and the lane's remote `spec/<m>/T<id>`; safe twice, unforced, asks nothing.
