...
| fleet-orchestrator | parent | session | local | none | required | One project's whole package, dispatched by the fleet at `--budget 1`. Local by construction: its queue is auto-memory, which no pushed repo carries. |
<!-- /tk:roles -->
