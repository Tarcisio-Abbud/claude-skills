...
An item's **block** ends at the first blank line, heading or next item; its fields live
there. Paragraphs below that blank are the item's only when they end at the next item:
`done`, `cancel`, `bump` and `migrate` move them with it, and the log gets them indented
under the entry, blank lines dropped. Prose running into a heading, a rule (a thematic break
or setext underline, never a lone `-`) or the end of the file is the section's: it stays,
and the command names it on stderr. A replayed close leaves the paragraphs in the queue
unless the entry already written carries them.
