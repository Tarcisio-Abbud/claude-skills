...
commit that repaired it has caught a defect born in the repair six times running. The loop that
does it — rounds, budget, design signal, inventory — is `../skills/review/SKILL.md`; where the
two differ, that file wins.
...
gap in your reading returns as its approval. One such run approved a slice that was missing four
binding requirements, all of them living in the comments. Give the subagent
`gh api "repos/<owner>/<repo>/issues/<n>/comments" --paginate`, with owner and repo spelled out.
The `{owner}/{repo}` shorthand gh accepts resolves from the working directory's remote, and the
